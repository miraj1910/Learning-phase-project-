...
for (var i = 0; i < trail.length - 1; i++) {
  ctx.beginPath();
  ctx.arc(
    trail[i].x * gridSize + gridSize / 2,  // center X
    trail[i].y * gridSize + gridSize / 2,  // center Y
    gridSize / 2 - 2,                      // radius
    0, Math.PI * 2
  );
  ctx.fill();
  ...
  ctx.fillStyle = 'lime'; // lighter tail
}
...
ctx.beginPath();
ctx.arc(
  trail[trail.length - 1].x * gridSize + gridSize / 2,
  trail[trail.length - 1].y * gridSize + gridSize / 2,
  gridSize / 2 - 2,
  0, Math.PI * 2
);
ctx.fill();
...
document.addEventListener("touchend", function (e) {
  ...
  if (Math.abs(dx) > Math.abs(dy)) {
    if (dx > 0) Snake.action('right');
    else Snake.action('left');
  } else {
    if (dy > 0) Snake.action('down');
